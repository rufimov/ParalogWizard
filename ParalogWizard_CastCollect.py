...
for folder in os.listdir(path_to_data_HP):
    if os.path.isdir(f'{path_to_data_HP}/{folder}'):
        ...
        with open(f'{path_to_data_HPM}/HybPiper_contigs/{folder}_contigs.fasta', 'w') as contigs:
            for locus in os.listdir(f'{path_to_data_HP}/{folder}'):
                if os.path.isdir(f'{path_to_data_HP}/{folder}/{locus}') \
                        and os.path.exists(f'{path_to_data_HP}/{folder}/{locus}/{locus}_contigs.fasta'):
                    print(f'\tProcessing {locus}')
                    with open(f'{path_to_data_HP}/{folder}/{locus}/{locus}_contigs.fasta') as locus_contigs:
