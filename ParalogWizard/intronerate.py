...
def make_intron_supercontig(contig_info, gene, prefix, add_N=False):
    cap3contigs = SeqIO.to_dict(
        SeqIO.parse("../{}_contigs.fasta".format(gene), "fasta")
    )
    ...
    SeqIO.write(
        intron_supercontig,
        "sequences/intron/{}_supercontig.fasta".format(gene),
        "fasta",
    )
...
def main():
    ...
    parser.add_argument(
        "--prefix",
        help="Prefix of sample directory generated by HybSeqPipeline",
        required=True,
    )
    parser.add_argument(
        "--no-exonerate",
        help="Don't re-run exonerate, use existing intronerate gff files.",
        action="store_true",
        default=False,
    )
    parser.add_argument(
    ...
    with open("intron_stats.txt", "w") as intron_stats_file:
        ...
        for gene in genelist:
            if check_for_files(gene, prefix):
                ...
                exonless_contig = remove_exons(
                    "intronerate.gff",
                    "sequences/intron/{}_supercontig.fasta".format(gene),
                )
                SeqIO.write(
                    exonless_contig,
                    "sequences/intron/{}_introns.fasta".format(gene),
                    "fasta",
                )
                ...
                sys.stderr.write(
                    "ERROR: Files not found for {} gene {}!\n".format(prefix, gene)
                )
