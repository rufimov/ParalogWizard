...
"""
    ...
    def concat(self):
        # concat command
        parser = argparse.ArgumentParser(description="Concatenate input alignments")
        parser.add_argument(
            "-p",
            "--concat-part",
            dest="concat_part",
...
class FileParser:
    ...
    def phylip_parse(self):
        ...
        matches = re.finditer(
            r"^(\s+)?(\S+)\s+([A-Za-z*?.{}-]+)", self.in_file_lines, re.MULTILINE
        )
    ...
    def phylip_interleaved_parse(self):
        ...
        tax_chars_matches = re.finditer(
            r"^(\s+)?([0-9]+)[ \t]+([0-9]+)", self.in_file_lines, re.MULTILINE
        )
        name_matches = re.finditer(
            r"^(\s+)?(\S+)[ \t]+[A-Za-z*?.{}-]+", self.in_file_lines, re.MULTILINE
        )
    ...
    def nexus_interleaved_parse(self):
        ...
        for taxon_no in range(len(taxa)):
            ...
            records[taxa[taxon_no]] = (
                self.translate_ambiguous(full_length_sequence).replace("\n", "").upper()
            )
            counter += 1
    ...
    def partitions_parse(self):
        ...
        matches = re.finditer(
            r"^(\s+)?([^ =]+)[ =]+([\\0-9, -]+)", self.in_file_lines, re.MULTILINE
        )
...
class Alignment:
    ...
    def get_taxon_char_summary(self):
        # get summary of frequencies for all characters
        records = (
            self.append_count(char_dict)
            for taxon, char_dict in self.get_counts_from_parsed()
        )
        return records
    ...
    def get_sites_no_missing_ambiguous(self):
        ...
        no_missing_ambiguous_sites = [
            self.get_site_no_missing_ambiguous(column)
    ...
    def get_variable(self):
        ...
        variable = len(
            [site for site in self.no_missing_ambiguous if not self.all_same(site)]
        )
        return variable
    ...
    def get_missing_from_parsed(self):
        ...
        self.missing_records = sorted(
            [
                (
                    taxon,
                    self.get_missing_from_seq(seq),
                    self.get_missing_percent_from_seq(seq),
                )
    ...
    def get_missing_percent_from_seq(self, seq):
        ...
        missing_seq_percent = round(
            (self.get_missing_from_seq(seq) / self.get_alignment_length() * 100), 3
        )
...
class DNAAlignment(Alignment):
    ...
    def get_taxa_summary(self):
        ...
        new_data = [
            list(data_tupl) + list(atgc) + chars
            for data_tupl, atgc, chars in zipped_list
        ]
        return new_data
    ...
    def get_atgc_content(self):
        ...
        at_content = round(
            sum(atgc[0] for taxon, atgc in atgc_records) / self.get_taxa_no(), 3
        )
    ...
    def get_atgc_from_parsed(self):
        ...
        return sorted(
            [
                (taxon, self.get_atgc_from_seq(seq))
                for taxon, seq in self.parsed_aln.items()
            ]
        )
...
class MetaAlignment:
    ...
    def translate_dict(self, source_dict):
        ...
        for taxon, seq in sorted(source_dict.items()):
            translated_seq = self.translate_dna_to_aa(
                seq, translation_table, self.reading_frame
            )
            if "*" in translated_seq:
                print(
                    "WARNING: stop codon(s), indicated as *, found in {} sequence".format(
                        taxon
                    )
                )
    ...
    def get_translated(self, translation_table, reading_frame):
        if int(self.cores) == 1:
            translated_alignments = [
                self.translate_dict(alignment) for alignment in self.parsed_alignments
            ]
        elif int(self.cores) > 1:
            pool = mp.Pool(int(self.cores))
            translated_alignments = pool.map(
                self.translate_dict, self.parsed_alignments
            )

        return translated_alignments

    def trim_dict(self, alignment):
        trim_vector = alignment.get_trim_selection(
            self.trim_fraction, self.parsimony_check
        )
    ...
    def get_trimmed(self, trim_fraction, parsimony_check):
        if int(self.cores) == 1:
            trimmed_alignments = [
                self.trim_dict(alignment) for alignment in self.alignment_objects
            ]
        elif int(self.cores) > 1:
            pool = mp.Pool(int(self.cores))
    ...
    def remove_empty_sequences(self, split_alignment):
        ...
        new_alignment = {
            taxon: seq
            for taxon, seq in split_alignment.items()
            if self.remove_unknown_chars(seq)
        }
    ...
    def get_alignment_objects(self):
        ...
        if int(self.cores) == 1:
            alignments = [
                self.get_alignment_object(alignment) for alignment in self.in_files
            ]
        elif int(self.cores) > 1:
            pool = mp.Pool(int(self.cores))
        ...
        for alignment in self.alignment_objects:
            ...
            if self.check_align == True:
                equal = all(
                    x
                    == [
                        len(list(parsed.values())[i])
                        for i in range(0, len(list(parsed.values())))
                    ][0]
                    for x in [
                        len(list(parsed.values())[i])
                        for i in range(0, len(list(parsed.values())))
                ...
                if equal is False:
                    print(
                        "ERROR: Sequences in input are of varying lengths. Be sure to align them first."
                    )
                    sys.exit()
                ...
                for taxon, seq in alignment.items():
                    ...
                    for dictionary in elements:
                        new_seq = (
                            new_seq
                            + seq[
                                dictionary["start"] : dictionary["stop"] : dictionary[
                                    "stride"
                                ]
                            ]
                        )
    ...
    def get_replicate(self, no_replicates, no_loci):
        ...
        for replicate in range(no_replicates):
            ...
            try:
                ...
                print(
                    "ERROR: You specified more loci per replicate than there are in your input."
                )
                sys.exit()
    ...
    def get_concatenated(self, alignments):
        ...
        for alignment in alignments:
            ...
            alignment_name = (
                self.alignment_objects[partition_counter - 1].get_name().split(".")[0]
            )
    ...
    def remove_taxa(self, species_to_remove):
        ...
        for index, alignment in enumerate(self.parsed_alignments):
            aln_name, aln_dict = self.remove_from_alignment(
                alignment, species_to_remove, index
            )
            ...
            if aln_dict:
                ...
                print(
                    "ERROR: You asked to remove all taxa from the alignment "
                    + aln_name
    ...
    def write_concat(self, file_format):
        ...
        print(
            "Wrote concatenated sequences to "
            + file_format
            + " file '"
            + file_name
            + "'"
    ...
    def write_replicate(self, index, alignment, file_format, extension):
        ...
        file_name = (
            "replicate" + str(index + 1) + "_" + str(self.no_loci) + "-loci" + extension
        )
        self.file_overwrite_error(file_name)
        self.write_formatted_file(file_format, file_name, alignment)
    ...
    def write_split(self, index, item, file_format, extension):
        ...
        try:
            ...
            print(
                "WARNING: There was no data to write for file '"
                + file_name
                + "'. Perhaps a partition composed of missing data only?"
            )
            remove(file_name)
            raise ValueError
        ...
        if action == "concat":
            ...
            [
                ...
                for i, alignment in enumerate(
                    self.get_replicate(self.no_replicates, self.no_loci)
                )
            ]

            print(
                "Constructed "
                + str(self.no_replicates)
                + " replicate data sets, each from "
                + str(self.no_loci)
                + " alignments"
            )
            ...
            print(
                ...
                + " "
                + str(file_format)
                + " files from partitions provided"
            )
            ...
            if aln_no:
                print(
                    ...
                    + " "
                    + str(file_format)
                    + " files with reduced taxon set"
                )

        elif action == "translate":
            if self.data_type == "aa":
                print(
                    "ERROR: cannot translate; you said your alignment already contains amino acids"
                )
                sys.exit()
            translated_alignment_dicts = self.get_translated(
                self.genetic_code, self.reading_frame
            )
            length = len(self.alignment_objects)
            [
            ...
            trimmed_alignment_dicts = self.get_trimmed(
                self.trim_fraction, self.parsimony_check
            )
            length = len(self.alignment_objects)
            [
            ...
            print(
                ...
                "file(s) to have",
                self.trim_fraction,
                "minimum occupancy per alignment column",
            )
