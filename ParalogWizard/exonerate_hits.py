...
def initial_exonerate(proteinfilename, assemblyfilename, prefix):
    ...
    exonerate_command = (
        "exonerate -m protein2genome --showalignment no --showvulgar no -V 0 --ryo %s %s %s >%s"
...
def get_contig_order(prot):
    ...
    tuplist = [
        (
            prot["hit_start"][i],
            prot["hit_end"][i],
            float(prot["assemblyHits"][i].split(",")[0].split("_")[5]),
        )
    ...
    sorting_order = sorted(
        list(range(len(tuplist))), key=lambda k: sort_key(tuplist[k])
    )
...
def filter_by_percentid(prot, thresh):
    ...
    kept_indicies = [
        i for i in range(len(prot["percentid"])) if prot["percentid"][i] > thresh
    ]
...
def supercontig_exonerate(supercontig, protseq, prefix, thresh=55):
    ...
    supercontig_cds = [
        i
        for i in SeqIO.parse(proc.stdout, "fasta")
        if float(i.id.split(",")[4]) > thresh
    ]
    logger.debug(
        "Supercontig lengths: %s" % " ".join([str(len(x.seq)) for x in supercontig_cds])
    )
...
def subsume_supercontigs(supercontigs):
    ...
    logger = logging.getLogger("pipeline")
    supercontig_rangelist = [
        (int(x.id.split(",")[2]), int(x.id.split(",")[3])) for x in supercontigs
    ]
...
def fullContigs(prot, sequence_dict, assembly_dict, protein_dict, prefix, thresh=55):
    ...
    if numHits == 1:
        ...
        return str(
            sequence_dict[prot["assemblyHits"][0]].seq
        )  # If only one hit to this protein.
        # else:
            ...
            if assembly_seq_name not in contigHits:  # Only add each contig once.
                if prot["hit_strand"][hit] == "+":
                    ...
                    sequence_list.append(
                        SeqRecord(
                            assembly_dict[assembly_seq_name].reverse_complement().seq,
    ...
    supercontig = SeqRecord(
        Seq("".join(str(b.seq) for b in sequence_list)), id=prot["name"]
    )
    logger.debug(">supercontig\n{}".format(supercontig.seq))
    # Need to remove contigs if they have the same basename
    supercontig_cds = supercontig_exonerate(
        supercontig, protein_dict[prot["name"]], prefix, thresh
    )
    ...
    SeqIO.write(
        joined_supercontig_cds, "%s/supercontig_exonerate.fasta" % prefix, "fasta"
    )
    ...
    superdupercontig = SeqRecord(
        Seq("".join(str(b.seq) for b in joined_supercontig_cds)), id=prot["name"]
    )
...
def overlapping_contigs(prot, length_pct, depth_multiplier):
    ...
    logger = logging.getLogger("pipeline")
    range_list = [
        (prot["hit_start"][i], prot["hit_end"][i])
        for i in range(len(prot["hit_start"]))
    ]

    logger.debug(range_list)
    kept_indicies = range_connectivity(
        range_list,
        prot["assemblyHits"],
        prot_length=prot["reflength"],
...
def best_by_percent_id(assemblyHits, full_length_indicies):
    ...
    for i in range(len(full_length_indicies)):
        ...
        if percentid > max_percentid:
            logger.debug(
                "percent_id: {}, maxpercent_id: {}".format(percentid, max_percentid)
            )
...
def best_by_depth(assemblyHits, full_length_indicies, thresh=10):
    ...
    for i in range(len(full_length_indicies)):
        depths.append(
            (
                full_length_indicies[i],
                float(
                    assemblyHits[full_length_indicies[i]].split(",")[0].split("_")[5]
                ),
            )
        )
...
def range_connectivity(
    ...
    if assemblyHits:
        if len(full_length_indicies) > 1:
            if use_depth:
                to_keep = best_by_depth(
                    assemblyHits, full_length_indicies, depth_multiplier
                )
                if to_keep:
                    return [to_keep]
                else:
...
def reciprocal_best_hit(prot, proteinHits):
    ...
    for contig in prot["assemblyHits"]:
        ...
        for otherProt in proteinHits:
            ...
            otherprot_contiglist = [
                x.split(",")[0] for x in proteinHits[otherProt]["assemblyHits"]
            ]
            if proteinHits[otherProt]["name"] != protname:
                if contigname in otherprot_contiglist:
                    full_contigname = [
                        b
                        for b in proteinHits[otherProt]["assemblyHits"]
                        if contigname in b
                    ][0]
                    logger.debug("%s %s" % (contig, full_contigname))
                    otherHit_idx = proteinHits[otherProt]["assemblyHits"].index(
                        full_contigname
                    )
                    ...
                    target_ranges = [
                        sorted(
                            (
                                prot["target_begin"][contig_idx],
                                prot["target_end"][contig_idx],
                            )
                        ),
                        sorted(
                            (
                    ...
                    if tuple_overlap(target_ranges[0], target_ranges[1]):
                        logger.debug(
                            "%s %s"
                            % (
                                repr(prot["percentid"][contig_idx]),
                                repr(proteinHits[otherProt]["percentid"][otherHit_idx]),
                            )
                        )
                        if (
                            prot["percentid"][contig_idx]
                            < proteinHits[otherProt]["percentid"][otherHit_idx]
                        ):
                            logger.debug(
                                "contig %s is a better hit to %s"
                                % (contigname, otherProt)
                            )
...
def paralog_test(exonerate_hits, prot, prefix):
    ...
    protlength = len(prot)
    hitlengths = [
        abs(int(x.split(",")[2]) - int(x.split(",")[3]))
    ...
    if sum(longhits) > 1:
        sys.stderr.write(
            "WARNING: Multiple long-length exonerate hits for {}. Check for paralogs!\n".format(
                prot.id
            )
        )
        with open("{}/paralog_warning.txt".format(prefix), "w") as pw:
            for hit in range(len(exonerate_hits["assemblyHits"])):
                if longhits[hit]:
                    pw.write(
                        prot.id + "\t" + exonerate_hits["assemblyHits"][hit] + "\n"
                    )
...
def help():
    ...
    print(
        "A protein and a nucleotide directory will be created in the current directory with the prefix."
    )
    return
...
def main():
    ...
    parser.add_argument(
        "proteinfile", help="FASTA file containing one 'bait' sequence per protein."
    )
    parser.add_argument(
        "assemblyfile", help="FASTA file containing DNA sequence assembly."
    )
    parser.add_argument(
        "--prefix",
        help="""Prefix for directory, files, and sequences generated from this assembly. 
            If not specified, will be extracted from assembly file name.""",
        default=None,
    )
    parser.add_argument(
        "--no_sequences",
    ...
    parser.add_argument(
        "--first_search_filename",
        help="Location of previously completed Exonerate results. Useful for testing.",
        default="no",
    )
    parser.add_argument(
        "-t",
        "--threshold",
        help="Threshold for Percent Identity between contigs and proteins. default = 55%%",
        default=55,
        type=int,
    )
    parser.add_argument(
        "--length_pct",
        help="Include an exonerate hit if it is at least as long as X percentage of the reference protein length. Default = 100%%",
        default=90,
        type=int,
    )
    parser.add_argument(
        "--depth_multiplier",
        help="Accept any full-length hit if it has a coverage depth X times the next best hit. Set to zero to not use depth. Default = 10",
        default=10,
        type=int,
    )
    ...
    if os.path.exists(args.first_search_filename):  # Shortcut for Testing purposes
        logger.info(
            "Reading initial exonerate results from file {}.".format(
                first_search_filename
            )
        )
    ...
    sys.stderr.write(
        "There were {} exonerate hits for {}.\n".format(
            len(sequence_dict), proteinfilename
        )
    )
    ...
    for prot in proteinHits:
        ...
        logger.debug(
            "After get_contig_order: %d" % len(proteinHits[prot]["assemblyHits"])
        )
        ...
        logger.debug(
            "After filter_by_percent_id: %d" % len(proteinHits[prot]["assemblyHits"])
        )
        if len(proteinHits[prot]["assemblyHits"]) == 0:
            report_no_sequences(proteinHits[prot]["name"])
            continue  # All hits have been filtered out
        ...
        proteinHits[prot] = overlapping_contigs(
            proteinHits[prot], args.length_pct * 0.01, args.depth_multiplier
        )
        #         logger.debug("After overlapping_contigs: %s" % " ".join(proteinHits[prot]["assemblyHits"]))
        logger.debug(
            "After overlapping_contigs: %d" % len(proteinHits[prot]["assemblyHits"])
        )
        ...
        if nucl_sequence:
            if args.no_sequences:
                ...
                sys.stderr.write(
                    "Writing amino acid sequence, length: {}\n".format(
                        len(amino_sequence)
                    )
                )
                sys.stdout.write(
                ...
                amino_filename = "%s/sequences/FAA/%s.FAA" % (
                    prefix,
                    prot.split("-")[-1],
                )
                ...
                nucleo_filename = "%s/sequences/FNA/%s.FNA" % (
                    prefix,
                    prot.split("-")[-1],
                )
