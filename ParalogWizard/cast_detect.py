...
def create_reference_wo_paralogs(
    ...
    exons: Set[str] = set()
    with open(
        os.path.join(path_to_data, "new_reference_for_HybPhyloMaker.fas"), "w"
    ) as new_reference_HPM, open(
        os.path.join(path_to_data, "new_reference_for_HybPiper_separate_exons.fas"), "w"
        ...
        for hit in all_hits_for_reference_scored:
            ...
            if sample not in blocklist:
                if exon(hit) not in exons:
                    ...
                    exons.add(exon(hit))
    with open(
        os.path.join(path_to_data, "new_reference_for_HybPiper_separate_exons.fas")
    ) as fasta_to_concatenate, open(
        os.path.join(path_to_data, "new_reference_for_HybPiper_concatenated_exons.fas"),
        "w",
...
def refine_phasing(
    ...
    if len(warn) > 0:
        with open(os.path.join(path_to_data, "warnings.txt"), "w") as warnings:
            warnings.write(
                ...
                if "para" in final_dict_to_check[key].keys():
                    if (
                        ...
                        if item not in final_dict_to_check[key]["para"].keys():
                            ...
                            warn.append(f"{key} {item}\n")
        if len(warn) > 0:
            with open(os.path.join(path_to_data, "warnings.txt"), "a") as warnings:
                warnings.write(
