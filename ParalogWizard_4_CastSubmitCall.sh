...
#PBS -l select=1:ncpus=6:mem=256gb:scratch_local=3Tb
#PBS -N ParalogWizard-Call
#PBS -m abe
#PBS -j oe
...
for dir in "${path_to_data}"/100remapped/exons*; do
  ...
  cp "$dir"/*filtered_uniq_sorted* "${SCRATCHDIR}/${data}"/100remapped/"${subfolder}"
...
python3 ParalogWizard.py cast_call -d ${data}  -nc ${cpu} 

#Copy results back
cp -r "${data}"/100remapped/* "${path_to_data}"/100remapped
