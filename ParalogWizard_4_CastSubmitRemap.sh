#!/bin/bash
#PBS -l walltime=96:0:0
#PBS -l select=1:ncpus=2:mem=128gb:scratch_local=1Tb
#PBS -N ParalogWizard-Remap
#PBS -m abe
#PBS -j oe

# Clean-up of SCRATCH
trap 'clean_scratch' TERM EXIT
trap 'clean_scratch' TERM

#Move to scratch
cd "${SCRATCHDIR}"

#Copy file with settings from home and set variables from settings.cfg
cp -f $PBS_O_WORKDIR/ParalogWizard_Settings.cfg .
cp -f /auto/brno12-cerit/nfs4/home/rufimov/Crataegus_phylogeny_all/ParalogWizard_Settings.cfg .
. ParalogWizard_Settings.cfg
path_to_data=/storage/"${server}/home/${LOGNAME}/${data}"
source=/storage/"${server}/home/${LOGNAME}"/HybSeqSource
cpu=$TORQUE_RESC_PROC

#Add necessary modules
module unload python
module add python/python-3.7.7-gcc-8.3.0-t4loj4a
export PYTHONUSERBASE=/storage/${server}/home/${LOGNAME}/python37
export PATH=$PYTHONUSERBASE/bin:$PATH
export PYTHONPATH=$PYTHONUSERBASE/lib/python3.7/site-packages:$PYTHONPATH

module add mafft
module add bwa 
module add samtools/1.11 
module add bcftools/1.11


mkdir -p "${SCRATCHDIR}/${data}"/10deduplicated_reads
mkdir -p "${SCRATCHDIR}/${data}"/41detected_par
echo "${sample}" > "${data}"/10deduplicated_reads/samples_list.txt


cp "${path_to_data}"/10deduplicated_reads/"${sample}"* "${SCRATCHDIR}/${data}"/10deduplicated_reads/


# cp "${path_to_data}"/10deduplicated_reads/samples_list.txt "${SCRATCHDIR}/${data}"/10deduplicated_reads/

cp "${path_to_data}"/41detected_par/all_paralogs_for_reference.tsv "${SCRATCHDIR}/${data}"/41detected_par

cp "${source}/${customized_probes}" .
cp "${source}"/ParalogWizard.py .
cp -r "${source}"/ParalogWizard .



#Run script

python3 ParalogWizard.py cast_remap -d ${data} -pc ${customized_probes} -nc ${cpu} -e ${exon_length}

#Copy results back
cp -r "${data}"/100remapped "${path_to_data}"

cp *.log "${PBS_O_WORKDIR}"/

exit 0
